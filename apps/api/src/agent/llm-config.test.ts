...
import { llmModelKwargs, resolveLlmConfig } from "./llm-config.js";
...
test("binds each provider key to its own default endpoint and model", () => {
  assert.deepEqual(resolveLlmConfig({ DEEPSEEK_API_KEY: "deepseek-secret" }), {
    provider: "deepseek", apiKey: "deepseek-secret", baseURL: "https://api.deepseek.com", model: "deepseek-v4-flash",
  });
...
test("disables DeepSeek V4 thinking for structured tool calls", () => {
  assert.deepEqual(llmModelKwargs("deepseek"), { thinking: { type: "disabled" } });
  assert.deepEqual(llmModelKwargs("openai"), {});
});
